        ...
        DATA.dataset.forEach(row => {
            ...
            switch (row.estado_requisicion) {
                ...
                case 'Aprobada':
                    ...
                    break;
                case 'Entregada':
                    STATE = "info'>Entregada";
                    break;
                case 'Anulada':
                    STATE = "danger'>Anulada";
                    break;
                default:
                    break;
            }
...
const showMaterials = async (id) => {
    ...
    if (DATA.status) {
        SHOWMODAL.show();
        // Mostrar la fecha de la última requisición (del primer registro)
        if (DATA.dataset.length > 0) {
            document.getElementById('dateMaterials').textContent = DATA.dataset[0].ultima_fecha_requisicion;
        } else {
            document.getElementById('dateMaterials').textContent = '';
        }
        // Se recorre el conjunto de registros (dataset) fila por fila a través del objeto row.
        DATA.dataset.forEach(row => {
            // document.getElementById('datematerials').innerHTML = row.ultima_fecha_requisicion;
            MODAL_BODY.innerHTML += `
                <tr>                    
