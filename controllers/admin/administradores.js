...
const fillTable = async (form = null) => {
    ...
    if (DATA.status) {
        // ? Obtener el tipo de administrador actual solo una vez
        const DATA2 = await fetchData(USER_API, 'getUser');
        // ? Si el administrador es tipo 1, muestra los botones, si no, no muestra nada
            ...
            if (DATA2.type == 1) {
                ...
                <div class="col-lg-2 col-md-12 col-sm-12 text-center mt-3">
                    <div class="d-flex flex-column">
                        <button class="btn btn-outline-dark mb-2" onclick="openDelete(${row.id_administrador})">
                            <i class="bi bi-trash"></i> Eliminar Administrador
                        </button>
                    </div>
                </div>`;
            }
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-lg-7 col-md-8 col-sm-12 text-center">
                            <h5 class="">Nombre administrador</h5>
                            <p class="card-title ">${row.nombre_administrador} ${row.apellido_administrador}</p>
                            <h5 class="">Correo Administrador</h5>
                            <p class="card-text ">${row.correo_administrador}</p>
                            <h5 class="">Teléfono personal</h5>
                            <p class="card-text ">${row.telefono_administrador}</p>
                        </div>
                        ${typeAdmin}
                    </div>
                </div>
...
const openDelete = async (id) => {
    ...
    const RESPONSE = await confirmAction('¿Desea eliminar administrador de forma permanente?');
    // Se verifica la respuesta del mensaje.
    if (RESPONSE) {
        const RESPONSE2 = await confirmAction('Se eliminara el historial en los registros alterador, ¿Continuar?');
        if (RESPONSE2) {
