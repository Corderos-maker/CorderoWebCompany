...
const PEDIDO_HERRAMIENTA_API = 'services/public/pedido_herramienta.php';
...
const loadTemplate = async () => {
    ...
    if (DATA.session) {
        ...
        if (DATA.status) {
            let alertaBoton = '';
            const DATA2 = await fetchData(REQUISICION_API, 'readDetail');
            if (DATA2.status) {
                alertaBoton = `
                <span class="position-absolute top-0 start-100 translate-middle p-2 bg-danger border border-light rounded-circle">
                    <span class="visually-hidden">New alerts</span>
                </span>`
                sweetAlert(4, 'Tiene una requisición en pendiente', true);
            } else {
                alertaBoton = '';
            }

            const DATA3 = await fetchData(PEDIDO_HERRAMIENTA_API, 'readDetail');
            if (DATA3.status) {
                alertaBoton2 = `
                <span class="position-absolute top-0 start-100 translate-middle p-2 bg-danger border border-light rounded-circle">
                    <span class="visually-hidden">New alerts</span>
                </span>`
                sweetAlert(4, 'Tiene un pedido de herramientas pendiente', true);
            } else {
                alertaBoton2 = '';
            }
                        ...
                        <div class="offcanvas offcanvas-end" id="contentNavbar" aria-labelledby="offcanvasNavbarLabel">
                            ...
                            <div class="offcanvas-body">
                                <ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link active" aria-current="page" href="../../views/public/bodega.html">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link active" aria-current="page" href="../../views/public/requisicion.html">
                                            <button type="button" class="btn btn-outline-dark w-100 position-relative">
                                                Requisición actual
                                                ${alertaBoton}
                                            </button>
                                        </a>
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link active" aria-current="page" href="../../views/public/pedido_herramienta.html">
                                            <button type="button" class="btn btn-outline-dark w-100 position-relative">
                                                Pedido Herramientas
                                                ${alertaBoton2}
                                            </button>
